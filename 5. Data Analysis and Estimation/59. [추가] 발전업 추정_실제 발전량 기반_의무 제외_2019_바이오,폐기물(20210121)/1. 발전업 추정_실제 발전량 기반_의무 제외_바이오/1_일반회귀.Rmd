---
title: "매출액 회귀선 및 예상 총 매출액 도출 (일반회귀)_바이오"
output:
  html_document:
    toc: yes
---
...
for(i in a){
      ...
      tmp_lm <- lm(신재생에너지_내수_2019년_기준 ~ REC_발급_발전량_2019년_기준, data = tmp_dat1)
      ...
      data.frame(항목 = c("에너지 원",
                   ...
                   내용 = c(i,
                          paste("E(발전매출액_백만원) =", round(coef_[1],7), "+", round(coef_[2],7),
                                "* REC_발급_발전량_2019년_기준" ),
      ...
      tmp_dat1$잔차 <- tmp_dat1$신재생에너지_내수_2019년_기준 - (X1 %*% coef_)
...
for(i in a){
  cat("#################################################################\n")
      ...
      tmp_lm <- lm(신재생에너지종사자 ~ REC_발급_발전량_2019년_기준, data = tmp_dat1)
      coef_ <- tmp_lm$coefficients
      coef_[is.na(coef_)] <- 0
