---
title: "매출액 회귀선 및 예상 총 매출액 도출 (가중회귀)_폐기물"
output:
  html_document:
    toc: yes
---
...
dat1 <- read_xlsx("폐기물표본조사O(28).xlsx", sheet = 1)

dat1 <- dat1[, c(2, 3, 6, 22, 23, 25, 27, 29)]
names(dat1)[1] <- "에너지원"
names(dat1)[2] <- "용량_kW"
names(dat1)[3] <- "사업체명"
names(dat1)[4] <- "신재생에너지_산업_분류_코드"
names(dat1)[5] <- "신재생에너지_분류_코드"
names(dat1)[6] <- "REC_발급_발전량_2019년_기준"
names(dat1)[7] <- "신재생에너지종사자"
names(dat1)[8] <- "신재생에너지_내수_2019년_기준"
dat1$의무대상자여부 <- 0

# REC 발급 발전량 : 0인 row data는 삭제
dat1 <- dat1 %>% filter(REC_발급_발전량_2019년_기준 != 0)
...
#names(dat12)[2] <- "에너지원"
#names(dat12)[3] <- "용량_kW"
#names(dat12)[4] <- "REC_발급_발전량_2019년_기준"
#names(dat12)[5] <- "신재생에너지_내수_2019년_기준"
#names(dat12)[6] <- "신재생에너지종사자"
#dat12$신재생에너지_산업_분류_코드 <- dat12$신재생에너지_분류_코드 <- NA

# REC 발급 발전량 : 0인 row data는 삭제
#dat12 <- dat12 %>% filter(REC_발급_발전량_2019년_기준 != 0)
...
# 1. 폐기물표본조사O(28).xlsx
#dat11 <- read_xlsx("폐기물표본조사O(28).xlsx", sheet = 1)
#dat11 <- dat11[, c(2, 3, 4, 25, 6)]
#names(dat11)[1] <- "에너지원"
#names(dat11)[2] <- "용량_kW"
#names(dat11)[3] <- "설비코드"
#names(dat11)[4] <- "REC_발급_발전량_2019년_기준"
#names(dat11)[5] <- "사업체명"
#dat11$의무대상자여부 <- 0


# 2. 폐기물표본조사X(7).xlsx
dat12 <- read_xlsx("폐기물표본조사X(7).xlsx", sheet = 1, skip = 2)

dat12 <- dat12[, c(2, 3, 4, 5, 8, 10)]
names(dat12)[1] <- "에너지원"
names(dat12)[2] <- "용량_kW"
names(dat12)[3] <- "설비코드"
names(dat12)[4] <- "의무대상자여부"
names(dat12)[5] <- "REC_발급_발전량_2019년_기준"
names(dat12)[6] <- "사업체명"
...
# REC 발급 발전량 : 0인 row data는 삭제
dat2 <- dat2 %>% filter(REC_발급_발전량_2019년_기준 != 0)

dat21 <- dat2[dat2$의무대상자여부==0,]
dat22 <- dat2[dat2$의무대상자여부==1,]

dat22 <- dat22 %>% group_by(에너지원,사업체명,의무대상자여부) %>% summarize(REC_발급_발전량_2019년_기준=sum(REC_발급_발전량_2019년_기준), 설비코드=NA, 용량_kW=NA)
...
```{r}
tmptmp1 <- dat1 %>% group_by(.,의무대상자여부,에너지원) %>% summarize(sum(REC_발급_발전량_2019년_기준))
tmptmp2 <- dat2 %>% group_by(.,의무대상자여부,에너지원) %>% summarize(sum(REC_발급_발전량_2019년_기준))
...
```{r}
tmptmp1 <- dat1 %>% group_by(.,의무대상자여부,에너지원_구분1) %>% summarize(sum(REC_발급_발전량_2019년_기준))
tmptmp2 <- dat2 %>% group_by(.,의무대상자여부,에너지원_구분1) %>% summarize(sum(REC_발급_발전량_2019년_기준))
...
```{r}
tmptmp1 <- dat1 %>% group_by(.,의무대상자여부,에너지원_구분2) %>% summarize(sum(REC_발급_발전량_2019년_기준))
tmptmp2 <- dat2 %>% group_by(.,의무대상자여부,에너지원_구분2) %>% summarize(sum(REC_발급_발전량_2019년_기준))
...
for(i in a){
      ...
      w_s <- (1/tmp_dat1$REC_발급_발전량_2019년_기준)
      tmp_lm <- lm(신재생에너지_내수_2019년_기준 ~ REC_발급_발전량_2019년_기준, data = tmp_dat1, weights = w_s)
      coef_ <- tmp_lm$coefficients
      coef_[is.na(coef_)] <- 0
      
      미의무_조사_원래_총_매출액 <- sum(tmp_dat1 %>% filter(.,의무대상자여부 == 0) %>% {.$신재생에너지_내수_2019년_기준})
      의무_조사_원래_총_매출액 <- sum(tmp_dat1 %>% filter(.,의무대상자여부 == 1) %>% {.$신재생에너지_내수_2019년_기준})
      ...
      data.frame(항목 = c("에너지 원",
                   ...
                   내용 = c(i,
                          paste("E(신재생에너지_내수_2019년_기준) =", round(coef_[1],7), "+",
                                round(coef_[2],7), "* REC_발급_발전량_2019년_기준" ),
      ...
      X1 <- cbind(1,tmp_dat1$REC_발급_발전량_2019년_기준)
      tmp_dat1$잔차 <- (w_s^.5)*(tmp_dat1$신재생에너지_내수_2019년_기준 - (X1 %*% coef_))
      tmp_dat1$abs_잔차 <- abs(tmp_dat1$잔차)
      
      print(ggplot(data = tmp_dat1, aes(x = REC_발급_발전량_2019년_기준,
                                        y = 신재생에너지_내수_2019년_기준)) +
...
for(i in a){
  cat("#################################################################\n")
      ...
      w_s <- (1/tmp_dat1$REC_발급_발전량_2019년_기준)
      tmp_lm <- lm(신재생에너지종사자 ~ REC_발급_발전량_2019년_기준, data = tmp_dat1, weights = w_s)
      coef_ <- tmp_lm$coefficients
      coef_[is.na(coef_)] <- 0
