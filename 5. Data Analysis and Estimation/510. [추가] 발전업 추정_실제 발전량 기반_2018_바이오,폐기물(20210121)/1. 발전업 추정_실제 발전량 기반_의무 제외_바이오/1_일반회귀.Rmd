...
dat1 <- read_xlsx("바이오표본조사O(44).xlsx", sheet = 1)

dat1 <- dat1[, c(2, 3, 5, 8, 9, 11, 13, 12)]
names(dat1)[1] <- "에너지원"
names(dat1)[2] <- "용량_kW"
names(dat1)[3] <- "사업체명"
...
# 바이오표본조사X(14).xlsx
dat12 <- read_xlsx("바이오표본조사X(14).xlsx", sheet = 1, skip = 1)
...
for(i in a){
      ...
      tmp_lm <- lm(신재생에너지_매출액_2018년_기준 ~ REC_발급_발전량_2018년_기준, data = tmp_dat1)
      ...
      tmp_dat1$잔차 <- tmp_dat1$신재생에너지_매출액_2018년_기준 - (X1 %*% coef_)
...
for(i in a){
  cat("#################################################################\n")
      ...
      tmp_lm <- lm(신재생에너지종사자 ~ REC_발급_발전량_2018년_기준, data = tmp_dat1)
      coef_ <- tmp_lm$coefficients
      coef_[is.na(coef_)] <- 0
