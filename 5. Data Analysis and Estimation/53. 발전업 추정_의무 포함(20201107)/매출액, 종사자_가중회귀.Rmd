...
dat1 <- dat1[, c(1, 2, 3, 6, 22, 23, 27, 26)]
names(dat1)[1] <- "출처"
names(dat1)[2] <- "에너지원"
names(dat1)[3] <- "용량_kW"
names(dat1)[4] <- "사업체명"
names(dat1)[5] <- "신재생에너지_산업_분류_코드"
names(dat1)[6] <- "신재생에너지_분류_코드"
names(dat1)[7] <- "발전매출액_백만원"
names(dat1)[8] <- "신재생에너지종사자"
dat1$의무대상자여부 <- 0
...
dat13$사업체명 <- "한국농어촌공사"
dat13 <- dat13 %>% group_by(., 출처, 에너지원, 사업체명, 신재생에너지_산업_분류_코드, 신재생에너지_분류_코드, 의무대상자여부) %>% summarize(.,용량_kW = sum(용량_kW), 발전매출액_백만원 = sum(발전매출액_백만원), 신재생에너지종사자 = sum(신재생에너지종사자))
...
dat2 <- read_xlsx("발전업2020(표본추출).xlsx", sheet = 2, skip = 3)

dat2 <- dat2[, c(2, 3, 4, 5, 6, 13)]
names(dat2)[1] <- "에너지원"
names(dat2)[2] <- "용량_kW"
names(dat2)[3] <- "설비코드"
names(dat2)[4] <- "사업년도"
...
for(i in a){
      ...
      tmp_lm <- lm(발전매출액_백만원 ~ 용량_kW, data = tmp_dat1, weights = w_s)
      ...
      tmp_dat1$잔차 <- (w_s^.5)*(tmp_dat1$발전매출액_백만원 - (X1 %*% coef_))
...
for(i in a){
  cat("#################################################################\n")
      ...
      w_s <- (1/tmp_dat1$용량_kW)
      tmp_lm <- lm(신재생에너지종사자 ~ 용량_kW, data = tmp_dat1, weights = w_s)
      coef_ <- tmp_lm$coefficients
      coef_[is.na(coef_)] <- 0
