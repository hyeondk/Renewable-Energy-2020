...
## 전체 매출액 다 한꺼번에 dot plot(상위업체 제외)
...
```{r}
library(ggplot2)
ggplot(proc_조사결과, aes(x = 총_매출액_2018)) + geom_dotplot(method="histodot", binwidth = 400) + theme_bw()
```
...
# 비중 불러들이기(추가된 코드)
중복_KSIC_매출 <- read_xlsx("C:/Users/mazy4/Desktop/소분류별_상위기업제외/매출액_소분류별/중복_KSIC_data_매출액_표1.xlsx")
...
for(소_i in 소_unique){
  ...
  # 매출액 수정(추가된 코드)
  temp1 <- 중복_KSIC_매출[중복_KSIC_매출$소분류 == 소_i,]
  총_매출 <- c(tmp_proc_조사결과$총_매출액_2018, temp1$매출액_수정)
  ...
  if(dim(tmp_proc_조사결과)[1] == 0){
    data.frame(
      ...
      표본_평균_매출액 <- mean(총_매출) # 총_매출 -> 변수 수정하였음.
      추정_총_매출액 <- 표본_평균_매출액*(sum(tmp_소분류별_KSIC_count_사업체수$사업체수추정) - sum(1/tmp_소분류별_KSIC_count_사업체수$가중치_n역수_)) # sum 부분 수정
      data.frame(
        항목 = c(
          "소분류",
          "매칭 에너지원",
      ...
      print(
        ggplot(
          tmp_proc_조사결과, aes(x = 총_매출액_2018)) +
          geom_dotplot(method="histodot") +
          geom_vline(xintercept = 표본_평균_매출액, linetype = 2) + 
          theme_bw()
        )
      tmp_df <- rbind(
        tmp_df,
...
소분류별_추정사업체수[,1:3] %>%
  ...
  merge(., tmp_df) %>%
  group_by(., 대, 중, 에너지원) %>%
  summarise(., 추정_매출액 = sum(추정_총_매출액 * 가중치)) %>%
  kable() %>% kable_styling()
```
